...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(
                ...
                "ERROR: expected END IF or ELSE");

        //handle if its else or end
        if (tokens.front().equals("ELSE")) {
            tokens.dequeue();
            Statement ifElseBlock = s.newInstance();
            Reporter.assertElseFatalError(tokens.length() > 0,
                    "ERROR: tokens is empty");
            ifElseBlock.parseBlock(tokens);
            Reporter.assertElseFatalError(
                    tokens.length() > 1 && tokens.dequeue().equals("END")
                            && tokens.dequeue().equals("IF"),
                    "ERROR: expected END IF, actual " + tokens.front());
            s.assembleIfElse(cond, ifBlock, ifElseBlock);
        } else {
            token = tokens.dequeue();
            Reporter.assertElseFatalError(
                    tokens.length() > 0 && tokens.dequeue().equals("IF"),
                    "ERROR: expected IF, actual " + token);
    ...
    private static void parseWhile(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(tokens.front().equals("DO"),
                "ERROR: expected DO, actual " + tokens.front());
        tokens.dequeue();
        ...
        Reporter.assertElseFatalError(tokens.dequeue().equals("END")
                && tokens.dequeue().equals("WHILE"), errorMessage);
    ...
    public void parse(Queue<String> tokens) {
        ...
        if (tokens.front().equals("IF")) {
            ...
            Reporter.fatalErrorToConsole("expected statement: EMPTY");
        }
    }
    ...
    public void parseBlock(Queue<String> tokens) {
        ...
        for (int i = 0; Tokenizer.isIdentifier(token) || token.equals("IF")
                || token.equals("WHILE"); i++) {
            Statement addMe = this.newInstance();
            addMe.parse(tokens);
            this.addToBlock(i, addMe);
            token = tokens.front();
        }
    }
